...
def recommend_grocery_meta(parent_asin, amazon_indices, amazon_match_indices, amazon_match_scores, df_amazon):
    if parent_asin not in amazon_indices:
        return pd.DataFrame(columns=['parent_asin', 'title', 'store', 'similarity_score'])
        
    # Grab the integer row position of our selected item
    idx = amazon_indices[parent_asin]
    if isinstance(idx, pd.Series):
        idx = idx.iloc[0]
        
    # Pull the pre-calculated top 5 matching rows and scores
    target_row_positions = amazon_match_indices[idx]
    target_similarity_scores = amazon_match_scores[idx]
    
    # Extract those exact rows via .iloc and assign the scores column
    results_df = df_amazon.iloc[target_row_positions].copy()
    results_df['similarity_score'] = target_similarity_scores.tolist()
